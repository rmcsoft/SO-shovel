(function () {
    ...
    let getNormalizedDump = function () {
        ...
        let postQuery = PostModel
            .find({ 'score': { '$gt': 0 } })
            .find({ 'postTypeId': { '$eq': 1 } });

        if (searchTags) {
            searchByTags = { 'tags': { '$regex': '.*' + searchTags + '.*' } };
            postQuery.find(searchByTags);
        };

        if (questionsScoreThreshold) {
            let scoreSearch = {
                'score': { $gt: questionsScoreThreshold }
            };
            postQuery.find(scoreSearch);
        }

        if (questionsFavoriteCount) {
            let questionsFavoriteCountSearch = {
                'favoriteCount': { $gt: questionsFavoriteCount }
            };
            postQuery.find(questionsFavoriteCountSearch);
        }
